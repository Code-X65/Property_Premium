    ...
    try {
      ...
      try {
        ...
        propertiesQuery = query(
          ...
          where('advertisingFor', '==', 'Sale'),
          where('PropertyCondition', '==', 'Newly Built'),
...
setIsShuffled(true); // Mark as shuffled
      ...
      try {
        ...
        querySnapshot.forEach((doc) => {
          const data = doc.data();
          // Filter for sale properties in JavaScript
          if (data.advertisingFor === 'Sale' && data.status === 'active' && data.propertyCondition === 'Newly Built' && data.propertyType === 'House') {
            propertiesData.push({
              id: doc.id,
              ...data,
              createdAt: data.createdAt?.toDate ? data.createdAt.toDate() : new Date(data.createdAt)
